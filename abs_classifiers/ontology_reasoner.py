...
class OntologyReasoner:
    ...
    def predict_sentiments(self, sentence_polarities, polarity_matrix):
    ...
    def run(self):
        ...
        results = {
            'classification model': "ontology reasoner",
            ...
            'cross_validations_accuracy_majority': [],
            'cross_validation_mean_accuracy_majority': "cross validation is switched off",
            'cross_validation_stdeviation_majority': "cross validation is switched off",
            ...
            'cross_validations_accuracy_with_backup': [],
            'cross_validation_mean_accuracy_with_backup': "cross validation is switched off",
            'cross_validation_stdeviation_with_backup': "cross validation is switched off"
        }
        ...
        if self.config.cross_validation:
            ...
            for i in range(self.config.cross_validation_rounds):
                ...
                majority_count_training, with_backup_count_training = self.predict_sentiments(
                ...
                majority_count_validation, with_backup_count_training = self.predict_sentiments(
                ...
                results['cross_validations_accuracy_majority'].append(accuracy_majority)
                results['cross_validations_accuracy_with_backup'].append(accuracy_with_back)

            results['cross_validation_mean_accuracy_majority'] = np.mean(results['cross_validations_accuracy_majority'])
            results['cross_validation_stdeviation_majority'] = np.std(results['cross_validations_accuracy_majority'])

            results['cross_validation_mean_accuracy_with_backup'] = np.mean(results['cross_validations_accuracy_with_backup'])
            results['cross_validation_stdeviation_with_backup'] = np.std(results['cross_validations_accuracy_with_backup'])
        ...
        majority_count_test, with_backup_count_test = self.predict_sentiments(
        ...
        #             self.predict_sentiment(self.sentence_vector[x], self.target_vector[x], self.onto, use_backup,
        #                                    use_svm, self.posinfo[x], types1, types2, types3)
        #
        ...
        #                 print(self.remaining_pos_vector)
        #                 outF = open("data/programGeneratedData/crossValidation" + str(
        #                     FLAGS.year) + '/svm/cross_val_remainder_' + str(j) + '.txt', "w")
        #                 with open(FLAGS.test_svm_path, "r") as fd:
        #                     for i, line in enumerate(fd):
        #                         if i in self.remaining_pos_vector:
        #                             outF.write(line)
