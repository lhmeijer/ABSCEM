...
class LCRRotHopModel(NeuralLanguageModel):
    ...
    def model_itself(self, left_sentence_part, right_sentence_part, target_part):
        ...
        left_hidden_state = bi_dynamic_rnn(cell, input_left, FLAGS.n_hidden, sen_len_fw, FLAGS.max_sentence_len,
                                           'l' + _id, 'all')
        ...
        right_hidden_state = bi_dynamic_rnn(cell, input_right, FLAGS.n_hidden, sen_len_bw, FLAGS.max_sentence_len,
                                            'r' + _id, 'all')
        pool_r = reduce_mean_with_len(right_hidden_state, sen_len_fw)


        # target hidden
        target = tf.nn.dropout(target_part, keep_prob=self.config.keep_prob1)
        target_hidden_state = bi_dynamic_rnn(cell, target, FLAGS.n_hidden, sen_len_tr, FLAGS.max_sentence_len,
                                             't' + _id, 'all')
        ...
        for iteration in range(self.config.n_iterations_hop):
            ...
            # attention target
            att_t_l = bilinear_attention_layer(target_hidden_state, left_context_representation, sen_len_tr,
            ...
            att_t_r = bilinear_attention_layer(target_hidden_state, right_context_representation, sen_len_tr,
        ...
        sentence_representation = tf.concat(
            [left_context_representation, target_left_context_representation, target_right_context_representation, right_context_representation], 1)
